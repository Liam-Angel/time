...
out = cv.VideoWriter('Anh/' + str(datetime.now()) + '.avi', fourcc, 10.0, (640, 480))
...
while True: 
  ...
  _, image = cv.threshold(image, 90, 255, cv.THRESH_BINARY)

  image = cv.Canny(image, 40,200)

  contours, hierarchy = cv.findContours(image,cv.RETR_EXTERNAL,cv.CHAIN_APPROX_SIMPLE)

  

  filter = [c for c in contours if (cv.minAreaRect(c)[1][0] * cv.minAreaRect(c)[1][1]) >10000]
  ...
  if pair[0] is not None:
    cv.circle(image, (rectx, recty), 10, (255,0,0),2)

    mapp = np.interp(int(angle), [0, 180], [0, 640])
    
    if rectx in range(290, 350):
      ser.write(bytes(str(int(mapp)) + '\n', encoding="utf-8"))
      cv.circle(image, (int(mapp), recty), 10, (255,255,0),2)
    else:
     ser.write(bytes(str(int(rectx)) + '\n', encoding="utf-8"))
     #print(rectx)
    ...
    r = 0
    for box in rects:
      if (box[1][0]*box[1][1]) > size and (box[1][0]*box[1][1]) < 30000:
    ...
    #rx = rects[0][0][0]
     ...
     # ser.write(bytes(str(int(mapp)) + '\n', encoding="utf-8"))
   # else:
     # mapp = np.interp(r, [0, 180], [320, 640])   
     # ser.write(bytes(str(int(mapp)) + '\n', encoding="utf-8"))

  
  cv.imshow('imageC', image)
